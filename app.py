...
try:
    importlib.reload(sys)  # Python 2.7
...
class InstagramScraper(object):
    ...
    def __scrape_query(self, media_generator, executor=concurrent.futures.ThreadPoolExecutor(max_workers=MAX_CONCURRENT_DOWNLOADS)):
        ...
        try:
            ...
            for value in self.usernames:
                ...
                while(iter <= self.maximum and not InstagramScraper.stop):

                    if self.include_location:
                        media_exec = concurrent.futures.ThreadPoolExecutor(max_workers=5)

                    iter = 0
                    items = tqdm.tqdm(media_generator(value), desc='Searching {0} for posts'.format(value), unit=" media",
                                          disable=self.quiet)
                    for item in items:

                        if ((item['is_video'] is False and 'image' in self.media_types) or \
                                    (item['is_video'] is False and 'video' in self.media_types)
                            ) and (self.is_new_media(item) and self.is_new_post(item)) :
                            # counter for number of media by owner
    ...
    def is_new_post(self, item):
        current_timestamp = self.__get_timestamp(item)
        epoch_day = 86400
        return current_timestamp >= (time.time() - epoch_day)

    def set_max_by_user(max):
        InstagramScraper.max_by_owner = max
...
def run(h_tag, maxPicbyUser,dst):
   ...
   # arguments = input("Enter command: ")
    arguments = h_tag+" --tag --maximum "+maxPicbyUser+ " --destination "+dst+" --retain-username"
